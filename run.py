...
def get_grounding_box(source_sentence, image_path, output_dir, box_threshold, 
              get_one_mask=True, with_logits=True):
  ...
  ##############################
    ...
    logits = outputs["encoder_logits"].sigmoid()[0]  # (seq length, num label=256) (nq, 256)
    boxes = outputs["encoder_pred_boxes"][0]  # (seq length, 4) (nq, 4)
