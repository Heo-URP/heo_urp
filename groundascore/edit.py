            ...
            with torch.enable_grad():
                ...
                for name, module in self.unet.named_modules():
                    ...
                    if module_name == "Attention":
                        # sa_cut
                        if "attn1" in name and "up" in name:
                            curr = module.hs #target 기준
                            ref = sa_attn[timestep.item()][name].detach().to(device) #source 기준
                            if curr.size(1) == 64**2:
                                bbox_scaled = bbox
...
def main(source_sentence, target_sentence, image_path, output_dir = "output/1111",num_iters = 500,model_id = "runwayml/stable-diffusion-v1-5",
    ...
    if masked_DDS:#run masked DDS
        ...
        if bbox == None:#make bbox using grounding dino
            raise ValueError("no grounding box")
    ...
    log_image_optimization_params(str(output_dir), text_source, text_target, num_iters, bbox, beta, cutloss_flag,str(image_path),reweight_flags)
